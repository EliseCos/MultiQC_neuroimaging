...
HARMONIZED_STATS_DATA = (
    "sample\tsite\tage\tsex\thandedness\tdisease\troi\tfa\nsub-01\tSite1\t30\tM\tR\tC\tBundle1\t0.45"
)
...
def test_data_dir():
    ...
    with open(os.path.join(tmpdir, "data_stats.tsv"), "w") as f:
        f.write(RAW_STATS_DATA)
    with open(os.path.join(tmpdir, "data.harmonized.tsv"), "w") as f:
        f.write(HARMONIZED_STATS_DATA)
    with open(os.path.join(tmpdir, "data.reference.tsv"), "w") as f:
    ...
    with open(os.path.join(tmpdir, "Site1.fa.raw.bhattacharyya.txt"), "w") as f:
        f.write(BHATT_RAW_DATA)
    with open(os.path.join(tmpdir, "Site1.fa.harmonized.bhattacharyya.txt"), "w") as f:
        f.write(BHATT_HARMONIZED_DATA)
...
def test_harmonization_module_filtering(reset_multiqc, test_data_dir):
    ...
    with open(os.path.join(test_data_dir, "Site1.md.raw.bhattacharyya.txt"), "w") as f:
        f.write(BHATT_RAW_DATA)
    ...
    # Configure filtering
    config.harmonization = {"metrics": "fa", "bundles": "Bundle1"}
    ...
    class MockDistSection:
        ...
        def filter_bundles(self, bundles):
            self.bundles = [b for b in self.bundles if b in bundles]

        def filter_metrics(self, metrics):
            self.metrics = [m for m in self.metrics if m in metrics]
    ...
    class MockBattSection:
        ...
        def build_html(self, *args, **kwargs):
            return type("HtmlContent", (), {"content": "", "metadata": {"render_metric_hook": "mock_batt_hook"}})()
