...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        dist_section_enabled = (
            len(ref_stats_file) > 0
            and len(raw_stats_files) > 0
            and len(harmonized_stats_files) > 0
            and len(data_model_plots_files) > 0
            and len(harmonization_plots_files) > 0
        )
        batt_section_enabled = len(harmonization_distance_files) > 0

        if not dist_section_enabled and not batt_section_enabled:
            log.debug(
                "Missing required files for harmonization module for the distribution section and Bhattacharyya section."
        ...
        if batt_section_enabled:
            batt_section = BattacharyyaSection(harmonization_distance_files)
        ...
        if len(bundles) == 0 and len(metrics) == 0:
            ...
            log.debug(
                "No bundle available for plotting after filtering. Skipping the harmonization distribution results section."
            )
        ...
        if dist_section_enabled:
            dist_html = dist_section.build_html(
                select_section.default_bundle, select_section.default_metric, select_section.render_plot_func
            )
